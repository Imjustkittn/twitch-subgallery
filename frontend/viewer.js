(() => {
  ...
  async function load() {
    try {
      const list = await fetchJSON('https://twitch-subgallery-1.onrender.com/api/photos');
  ...
  Twitch.ext.onAuthorized(async (_auth) => {
    ...
    Twitch.ext.listen('broadcast', (_topic, _type, msg) => {
      ...
      } else if (ev.type === 'comment:approved') { } } catch {};
    });
    Twitch.ext.bits.onTransactionComplete(async (tx) => {
      ...
      try { await fetchJSON('https://twitch-subgallery-1.onrender.com/api/transactions/complete', payload); }
